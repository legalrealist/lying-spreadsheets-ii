...
2. **The LLM's own numerical reasoning is a partial natural defense** — not safety, just cross-footing. It caught C1 and C2 because the fabricated value contradicted other visible cells.
3. **Full internal consistency defeats that defense (C3).** Because real financial models are formula-driven, an attacker can tamper a *coherent set* of caches so every cross-foot ties. The analyst then cannot distinguish fabrication from truth without recomputing formulas from precedents — which the pipeline never does. This reproduces the core lying-spreadsheets result at the cached-value layer.
